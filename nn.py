...
#
#
# def conv2d(x,kernel, image_shape=None, filter_shape=None, border_mode='valid',subsample=(1,1),**kargs):
#     strides=subsample
#     if _on_gpu() and dnn.dnn_available():
#         if border_mode == 'same':
#             np_kernel = kernel.eval()
#             assert strides[0] <= np_kernel.shape[2],'strides should be smaller than window'
#             assert strides[1] <= np_kernel.shape[3], 'strides should be smaller than window'
...
#             conv_out = conv_out[:, :, shift_x:shift_x+expected_width, shift_y:shift_y+expected_height]
#         else:
...
#         conv_out = T.nnet.conv2d(x,kernel,border_mode=th_border_mode, subsample=strides, image_shape=image_shape, filter_shape=filter_shape)
#         if border_mode == 'same':
#             shift_x = (np_kernel.shape[2] - strides[0]) // 2
#             shift_y = (np_kernel.shape[3] - strides[1]) // 2
#             expected_width = (x.shape[2] + strides[0] - 1) // strides[0]
...
class Convolution1d(object):
    def __init__(self, num_channels, win_len, name='convolution layer'):
        self.win_len = win_len
        self.num_channels = num_channels
        self.name = name
        self.convo = shared((num_channels, num_channels, win_len, 1), self.name + '__convolution')
        self.params = [self.convo]

    def link(self, input):
        assert input.ndim == 4 or input.ndim == 3
        if input.ndim == 3:
            input = input.dimshuffle('x', 0, 1, 2)
        char_shape = input.shape
        input = input.reshape((char_shape[0]*char_shape[1], char_shape[2], 1, char_shape[3]))
        self.input = input.dimshuffle(0, 3, 1, 2)
        wlen = self.convo.shape[2] // 2
        conv1d = T.nnet.conv.conv2d(self.input, self.convo, border_mode='full')
        conv1d = conv1d[:, :, wlen:-wlen, :]
        self.output = conv1d.dimshuffle(0, 2, 3, 1).reshape(char_shape)
        return self.output
    ...
    def link(self, input):
        ...
        def recurrence(x_t, c_tm1, h_tm1):
            ...
            o_t = T.nnet.sigmoid(T.dot(x_t, self.w_xo) +
                                 ...
                                 self.b_o)
            h_t = o_t * T.tanh(c_t)
            return [T.cast(c_t,'float32'), T.cast(h_t,'float32')]
